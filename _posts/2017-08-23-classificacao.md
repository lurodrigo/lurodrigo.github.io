...
relação de equivalência, uma relação $R$ que satisfaz as seguintes propriedades:

* É *reflexiva*: $ARA$, todo $A$ está relacionado (por $R$) a si mesmo.
* É *simétrica*: Para quaisquer $A$ e $B$, quando $ARB$, $BRA$ também. Isto é, quando A está relacionado a B, B está relacionado a A.
* É *transitiva*: Quando $ARB$ e $BRC$, $ARC$ também. Ou seja, quando A está ligado a B e B está ligado a C, A está ligado a C também.
...
qualquer critério vai pecar por falha ou excesso.
